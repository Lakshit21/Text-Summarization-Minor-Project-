...
def adversarial_train(generator, discriminator, generator_batcher, discriminator_batcher, generator_val_batcher, discriminator_val_batcher, summary_writer, sess_context_manager):
    ...
    with sess_context_manager as sess:
        ...
        while True:
            ...
            batch = generator_batcher.next_batch()
            batch_val = generator_val_batcher.next_batch()
            ...
            result_val = generator.run_eval_step(sess, batch_val)
            loss_val = result_val['loss']
            summaries_val = result_val['summaries']
            val_step = result_val['global_step']
            summary_writer.add_summary(summaries_val, val_step)
            print("validation step: %d validation loss: %.3f" % (val_step, loss_val))
