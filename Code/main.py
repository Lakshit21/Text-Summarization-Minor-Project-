...
tf.app.flags.DEFINE_string('validation_path', '', 'Path expression to text validation file.') 
...
def setup_training(mode, generator, discriminator, generator_batcher, discriminator_batcher, generator_val_batcher, discriminator_val_batcher):
...
def main(args):
    ...
    generator_val_batcher = Batcher(FLAGS.validation_path, vocab, hps, single_pass=FLAGS.single_pass)
    discriminator_val_batcher = Batcher(FLAGS.validation_path, vocab, hps, single_pass=FLAGS.single_pass)
